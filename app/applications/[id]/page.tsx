    ...
    <div className="min-h-screen bg-base">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 space-y-6">
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-surface p-6 sm:p-8">
          ...
          {scoreDetails?.critical_issues?.length > 0 && <div className="mt-4"><p className="text-xs font-semibold text-red-400 mb-1.5">Critical issues:</p><ul className="space-y-0.5">{scoreDetails.critical_issues.map((issue: string, i: number) => <li key={i} className="text-xs text-red-400/80 flex gap-2"><span className="shrink-0">!</span> {issue}</li>)}</ul></div>}
          {scoreDetails?.quick_wins?.length > 0 && <div className="mt-3"><p className="text-xs font-semibold text-emerald-400 mb-1.5">Quick wins:</p><ul className="space-y-0.5">{scoreDetails.quick_wins.map((win: string, i: number) => <li key={i} className="text-xs text-emerald-400/80 flex gap-2"><span className="shrink-0">+</span> {win}</li>)}</ul></div>}
        </div>
